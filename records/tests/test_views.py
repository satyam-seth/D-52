...
class TestExportAllRecordView(TestCase):
    """Test export all record view"""

    def setUp(self) -> None:
        self.export_view = ExportAllRecordView()

    def test_export_all_record_view_attributes(self) -> None:
        "Test export all record view attributes"

        self.assertIsInstance(self.export_view, BaseExportView)

    @mock.patch("records.views.User.objects.filter")
    @mock.patch("records.views.BaseExportView.get_room_and_export_instance")
    @mock.patch.object(ExportAllRecordView, "get_file_response")
    def test_export_all_record_view_post_working(
        self,
        mock_get_file_response,
        mock_get_room_and_export_instance,
        mock_user_filter,
    ):
        """Test export all record view post working"""
        ...
        # Assert that get_file_response was called with the correct file name and data
        expected_file_name = f"{mock_room_name}_all_records_data"
        expected_data = [
            ("All Records", pd.DataFrame({"data": [1, 2, 3]})),
        ]
...
class TestExportMaidView(TestCase):
    """Test export maid view"""
    ...
    @mock.patch("records.views.BaseExportView.get_room_and_export_instance")
    @mock.patch.object(ExportMaidView, "get_file_response")
    def test_export_maid_view_post_working(
        self,
        mock_get_file_response,
        mock_get_room_and_export_instance,
    ):
        """Test export maid view post working"""
        ...
        # Mocking the exporter's data method for maid records
        mock_exporter.get_maid_df.return_value = pd.DataFrame({"maid": [1, 2, 3]})
        ...
        # Assert that get_file_response was called with the correct file name and data
        expected_file_name = f"{mock_room_name}_maid_data"
        expected_data = [
            ("Maid Records", pd.DataFrame({"maid": [1, 2, 3]})),
        ]
...
class TestExportElectricityView(TestCase):
    """Test export electricity view"""

    def setUp(self) -> None:
        self.export_view = ExportElectricityView()

    def test_export_electricity_view_attributes(self) -> None:
        "Test export electricity view attributes"
